...
- apps/api/src/modules/upload/upload-gc.service.ts
...
- `class UploadGcService { garbageCollect(options: { dryRun?: boolean | undefined; minAgeHours?: number | undefined; }); scheduled(); collectReferencedKeys() }`
- `class UploadController { getPresignedUrl(body: { filename: string; contentType: string; folder?: string | undefined; }); deleteFile(body: { url: string; }); runGc(auth: string, dryRunQ: string | undefined, minAgeHoursQ: string | undefined); serveFile(folder: string, filename: string, res: Response<any, Record<string, any>>) }`
- `class UploadModule`
- `class UploadService { getS3(); getBucket(); generatePresignedUrl(filename: string, contentType: string, folder: string); getFile(key: string); extractKey(url: string); deleteFile(url: string); deleteFiles(urls: string[]); listAllObjects() }`
...
### Interfaces
- `interface GcResult { scanned: number; referenced: number; candidates: number; deleted: number; skippedTooYoung: number; dryRun: boolean }`
...
`@nestjs/common`, `@nestjs/config`, `@nestjs/throttler`, `@nestjs/jwt`, `@nestjs/passport`, `resend`, `bcryptjs`, `@imovdigital/utils`, `passport-jwt`, `class-validator`, `crypto`, `@imovdigital/types`, `@anthropic-ai/sdk`, `express`, `stripe`, `dns`, `@nestjs/schedule`, `@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`, `stream`
