    ...
    <div style={{ padding: isMobile ? '40px 16px' : '64px 32px' }} className="bg-white">
      <div className="max-w-6xl mx-auto">
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 32 : 48 }}>
          ...
          <div className="space-y-6" style={{ flex: 1 }}>
            {cd?.address && (
              <div className="flex items-start gap-3">
                ...
                <p className="text-gray-600 text-sm">{cd.address}</p>
              </div>
            )}
            ...
            {settings.showWhatsApp && cd?.whatsapp && (
              <div className="flex items-center gap-3">
                ...
                <p className="text-gray-600 text-sm">{cd.whatsapp}</p>
              </div>
            )}
          ...
          {/* Form */}
          {settings.showForm && (
            <div className="bg-gray-50 rounded-xl p-6" style={{ flex: 1 }}>
              <div className="space-y-4">
                <input
                  readOnly
                  placeholder="Seu nome *"
                  className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm outline-none text-gray-400 cursor-default"
                />
                {showEmailField && (
                  <input
                    readOnly
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm outline-none text-gray-400 cursor-default resize-none"
                />
